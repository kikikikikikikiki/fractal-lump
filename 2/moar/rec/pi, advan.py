...
surface_x = 800
surface_y = 800
main_surface = pygame.display.set_mode((900,800),pygame.HWSURFACE+pygame.FULLSCREEN)
...
def draw_tree(order, depthprint, theta, thetab, sz, posn, heading, color=(125,100,150), depth=0):
   ...
   delta_x = trunk * math.exp(math.cos(heading)//2*math.pi)
   delta_y = trunk * math.exp(math.sin(heading)//2*math.pi)
   (u, v) = posn
   ...
   if order > 0:   # Draw another layer of subtrees
      ...
      draw_tree(order-1, depthprint, theta*math.cos(heading), thetab*math.sin(heading), newsz, newpos, heading+theta, color1, order+math.sin(heading+theta))
      draw_tree(order-1, depthprint, theta*math.cos(heading), thetab*math.cos(heading), newsz, newpos, heading+theta+12, color2, order+math.sin(heading+theta+12))
#@lru_cache(maxsize=10000)
def gameloop():
    depthincr = 20
    theta1 = 1
    ...
    while True:
        ...
        main_surface.blit(text, textpos)
        draw_tree(orrder, int(depthprint), theta1, theta2, surface_size*0.8, (pygame.mouse.get_pos()[0], pygame.mouse.get_pos()[1]), 0,color=(0,0,0), depth=depthincr)
