...
def draw_tree(order, depthprint, theta, thetab, sz, posn, heading, color=(125,100,150), depth=0):
   ...
   delta_x = trunk * math.sin(math.log10(math.exp((heading+theta)/2)))
   ...
   if order > 0:   # Draw another layer of subtrees
      ...
      draw_tree(order-1, depthprint, theta*math.sin(heading), thetab*math.sin(heading), newsz, newpos, heading+theta, color1, order+math.sin(heading+theta))
      draw_tree(order-1, depthprint, theta*math.cos(heading), thetab*math.cos(heading), newsz, newpos, heading+theta+12, color2, order+math.cos(heading+theta))
#@lru_cache(maxsize=10000)
def gameloop():
    depthincr = 20
    theta1 = 1
