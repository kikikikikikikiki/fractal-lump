...
def draw_tree(order, depthprint, theta, thetab, sz, posn, heading, color=(125,100,150), depth=0):
   ...
   delta_x = trunk * math.cos(math.log10(math.exp(math.cos(theta))))
   delta_y = trunk * math.sin(math.log10(math.exp(heading + math.sin(theta))))
   (u, v) = posn
   ...
   if order > 0:   # Draw another layer of subtrees
      ...
      draw_tree(order-1, depthprint, theta*math.sin(heading), thetab*math.cos(heading), newsz, newpos, heading, color1, order+math.sin(heading+theta))
      draw_tree(order-1, depthprint, theta*math.sin(heading), thetab*math.cos(heading), newsz, newpos, heading+math.sin(heading+theta+5), color2, order+math.sin(heading+theta+5))
#@lru_cache(maxsize=10000)
def gameloop():
    depthincr = 20
    theta1 = 1
