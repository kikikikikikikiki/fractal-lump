...
def draw_tree(order, depthprint, theta, thetab, sz, posn, heading, color=(125,100,150), depth=0):
   ...
   delta_x = trunk * math.exp(math.sin((heading*1j).real))+math.cos(heading.imag)
   ...
   if order > 0:   # Draw another layer of subtrees
      ...
      draw_tree(order-1, depthprint, theta*math.cos(heading.real), thetab*math.sin(heading.real), newsz, newpos, (heading+thetaj), color1, depth+(theta*math.sin(heading.real)))
      draw_tree(order-1, depthprint, theta*math.cos(heading.real), thetab*math.sin(heading.real), newsz, newpos, (heading+thetaj+12), color2, depth+(theta*math.sin(heading.real)))
#@lru_cache(maxsize=10000)
def gameloop():
    depthincr = 0
    theta1 = 0
