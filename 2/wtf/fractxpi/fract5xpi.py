...
def draw_tree(gen, order, depthprint, theta, thetab, sz, posn, heading, color=(125,100,150), depth=0):
   ...
   trunk_ratio = 0.161803398875    # How big is the trunk relative to whole tree?
  ...
  # pygame.draw.polygon(main_surface, color, [[int(posn[0]),int(posn[1])], [(surface_size/2), (surface_size/2)], [(surface_size/2), (surface_size/2)]], 5)
   if order > 0:   # Draw another layer of subtrees
      ...
      draw_tree(gen, order-1, depthprint, theta*math.cos(heading), thetab*math.sin(heading), newsz, newpos, heading+theta+12, color1, depth+math.sin(heading+theta+12))
      draw_tree(gen, order-1, depthprint, theta*math.cos(heading), thetab*math.sin(heading), newsz, newpos, heading+theta-1, color2, depth+math.sin(heading+theta+1))
