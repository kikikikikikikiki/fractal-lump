...
def draw_tree(order, theta, thetab, sz, posn, heading, color=(0,0,0), depth=0):
   ...
   if order > 0:   # Draw another layer of subtrees
      ...
      draw_tree(order-1, theta*math.sin(heading), thetab*math.sin(heading), newsz, newpos, heading+theta*math.sin(heading), color1, ((depth+12)/15)*10)
      draw_tree(order-1, thetab*math.sin(heading), theta*math.sin(heading), newsz, newpos, heading-theta*math.cos(heading), color2, ((depth+12)/15)*10)
@lru_cache(maxsize=10000)
def gameloop():
    depthincr = 0
    theta1 = 0
