...
def draw_tree(order, theta, thetab, sz, step, clol, clol2, posn, heading, color=(0,0,0), depth=0):
   ...
   a = heading+(2j)
   b = theta+(2j)
   delta_x = trunk * math.sin(math.log10(math.exp(((a.real)*b.imag))))
   delta_y = trunk * math.cos(math.log10(math.exp(((a.imag)*b.real))))
   colx =  math.sin(math.log10(math.exp(((a.real)*b.imag))))
   coly = -math.cos(math.log10(math.exp(((a.imag)*b.real))))
   ...
   if order > 0:   # Draw another layer of subtrees
      ...
      # at other depths, or when depth is even, or odd, etc.
         color1 = (order**2, 125*(1+colx), 125*(1+coly))
         color2 = (order**2, 125*(1+colx), 125*(1+coly))
...
def gameloop():
    ...
    while True:
        ...
        draw_tree(8, theta1, theta2, surface_size*1.1, step, colol, colol2, pygame.mouse.get_pos(), -math.pi/2, color=(0,0,0), depth=depthincr)
