...
surface_size = 1000
main_surface = pygame.display.set_mode((400,225),pygame.DOUBLEBUF+pygame.FULLSCREEN)
...
def draw_tree(inord, order, theta, thetab, sz, posn, heading, color=(0,0,0), depth=0):
   ...
   gradd4 = 128+(127*math.cos((heading).real))
   ...
   if order > 0:
      if depth == 0:
          color1 = (gradd2,gradd,gradd4)
          color2 = (gradd2,gradd3,gradd4)
      else:
      ...
      #pygame.display.flip()
      draw_tree(inord, order-1, theta, thetab, newsz, newpos, (heading+theta+102j), color1, depth)
...
def gameloop():
    ...
    timer = wave.open('sawramp1.wav', 'rb')
       ...
       if math.fabs(decoded[0]) > 0:
           if math.fabs(decoded[1]) > 0:
               ...
               for i,x,l in zip(decoded_oh,decoded2,decoded4):
                  ...
                  theta1 = ((2*math.pi*((i/2147483647)))*(x/2147483647)*1600/225)**math.cos(math.pi*((i/2147483647)))#*((x/2147483647))#*(1*(pygame.mouse.get_pos()[0]/450))
                  theta2 = math.cos(2*math.pi*(math.exp((l/50)/2147483647)))+math.sin(-2*math.pi*(((l/50)/2147483647)))**math.cos(2*math.pi*((l/2147483647)))#*(1*(pygame.mouse.get_pos()[1]/450))
                  ...
                  draw_tree(inord,5, theta1, theta2, surface_size*(0.2+(0.2*(i/2147483647))), (200+(200*(i/2147483647)),112.5+(112.5*(i/2147483647))), (i/2147483647)*math.pi*(((l/5)/2147483647)))
