...
surface_size = 1000
main_surface = pygame.display.set_mode((3200,1800))#,pygame.DOUBLEBUF+pygame.FULLSCREEN)
...
def draw_tree(inord, order, theta, thetab, sz, posn, heading, color=(0,0,0), depth=0):
   ...
   delta_y = trunk * math.cos(heading.real)#calc_fourier_seriesx(heading)
   delta_x = trunk * math.sin(heading.real)#calc_fourier_seriesy(heading)
   thetaj = theta*1j*1j
   thetai = thetab*1j*1j
    ...
    def callback(in_data, frame_count, time_info, status):
       ...
       if math.fabs(decoded[0]) > 0:
           if math.fabs(decoded[1]) > 0:
               #print(decoded[0])
               for i,x in zip(decoded_oh,decoded2):
                  ...
                  theta1 = ((math.sin(2*math.pi*(i/2147483647)))*math.cos(2*math.pi*(i/2147483647)))**math.sin(2*math.pi*((i/2147483647)))**2#**(1*(pygame.mouse.get_pos()[0]/900))
                  theta2 = 0#pygame.mouse.get_pos()[0]/225#200j#2/2147483647#(math.pi)#math.sin(2*math.pi*((i.real**2/2147483647)))*1j**math.sin(2*math.pi*((i.imag/2147483647))))#*(1*(pygame.mouse.get_pos()[1]/225))
                  ...
                  draw_tree(inord,4, theta1, theta2, surface_size*1, (1600,450), ((x/2147483647)))
