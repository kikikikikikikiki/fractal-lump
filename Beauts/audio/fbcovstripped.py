...
main_surface = pygame.display.set_mode((1000*m,1000*m))#,pygame.DOUBLEBUF)#+pygame.FULLSCREEN)
my_clock = pygame.time.Clock()

lru_cache(maxsize=None)
def draw_tree(flip,inord, order, theta, thetab, sz, posn, heading, color=(0,0,0), depth=0):

   trunk_ratio = (1 + 5 ** 0.5) / 2       
   trunk = sz * trunk_ratio
   delta_y = trunk * ((calc_fourier_series_cos(heading)*400/400).real)
   delta_x = trunk * ((calc_fourier_series_sin(heading)*400/400).real)
   ...
   gradd  = 128+(127*math.sin((heading).real+heading.imag))
   gradd2 = 128+(127*math.sin((heading).real+heading.imag))
   gradd2 = 128+(128*math.sin((heading).real))
   gradd3 = 128+(127*math.sin(math.pi*calc_fourier_series_cos(heading)))
   ...
   if order > 0:
      if depth == 0:
          ...
          color1 = (gradd3,gradd3,gradd3)
          color2 = (gradd,gradd2,0)
      else:
      ...
      #pygame.display.flip()
      flip = 1
      draw_tree(flip, inord, order-1, theta, thetab, newsz, newpos, (heading+theta+12j), color1, depth)
      flip = 0
      draw_tree(flip, inord, order-1, theta, thetab, newsz, newpos, (heading+thetab+12j), color2, depth)
...
def calc_fourier_series_sin(heading):
    inc = 1
    something = 0
    for x in range(1):
...
def calc_fourier_series_cos(heading):
    inc = 1
    something = 0
    for x in range(100):
       ...
       if math.fabs(decoded[0]) > 0:
           if math.fabs(decoded[1]) > 0:
               ...
               for i,j in zip(decoded_ohh,decoded_oh):
                  ...
                  theta2 = -math.sin(2*math.pi*(i/2147483647))*1j#*(1*(pygame.mouse.get_pos()[0]/450))
                  ...
                  draw_tree(flip, inord, 2, theta1, theta2, surface_size*0.125, (500*m,500*m), (math.pi*(i/2147483647)))
