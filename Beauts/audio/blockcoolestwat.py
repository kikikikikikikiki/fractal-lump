...
def draw_tree(inord, order, theta, thetab, sz, posn, heading, color=(0,0,0), depth=0):
   ...
   delta_x = trunk * ((heading*400/400).imag)
   delta_y = trunk * ((heading*400/400).real)
       ...
       if math.fabs(decoded[0]) > 0:
           if math.fabs(decoded[1]) > 0:
               ...
               for i, j in zip(decoded_oh, yf_oh): # Change to every other value for left/right side [0] = left [1] = right
                  ...
                  theta1 = math.sin(2*math.pi*(i/32767))*1
                  theta2 = math.sin(2*math.pi*(i/32767))*1
                  ...
                  draw_tree(inord,6, theta1, theta2, surface_size*0.1, (300,300), 2*math.pi*(j/32768))
       #pygame.display.flip()
