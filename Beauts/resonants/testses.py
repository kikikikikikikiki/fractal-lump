...
def draw_treexy(arr, rendord,inord, order, theta, thetab, sz, clol, clol2, posn, heading, color=(0,0,0), depth=0):
   ...
   thetaj = theta + 9.87j
   theta2 = theta + 6.1j
   xxxx = (math.fabs(posn[0]+newpos[0])/2)
   xxxxx = (math.fabs(posn[1]+newpos[1])/2)
   xx = ((1600/2)-(xxxx/2))
   xxx = ((900/2)-(xxxxx/2))
   if xxxx>1:
      if xxxxx>1:
          if order<rendord:
             pygame.draw.arc(main_surface, color, [xx,xxx,(1+xxxx),(1+xxxxx)], 0, 2.2*math.pi, 1)
   if order > 0:   # Draw another layer of subtrees
...
def draw_tree12xy(arr,rendord,inord, order, theta, thetab, sz, clol, clol2, posn, heading, color=(0,0,0), depth=0):
   ...
   thetaj = theta + 9.87j
   theta2 = theta + 6.1j
   xxxx = (math.fabs(posn[0]+newpos[1])/2)
   xxxxx = (math.fabs(posn[1]+newpos[0])/2)
   xx = ((1600/2)-(xxxx/2))
   xxx = ((900/2)-(xxxxx/2))
   if xxxx>1:
      if xxxxx>1:
