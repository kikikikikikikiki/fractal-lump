...
def draw_treexy(arr, rendord,inord, order, theta, thetab, sz, clol, clol2, posn, heading, color=(0,0,0), depth=0):
   trunk_ratio = 1.6180327868852      # How big is the trunk relative to whole tree?
   trunk = sz * trunk_ratio # length of trunk
   delta_x = trunk * calc_fourier_seriesx(heading)
   delta_y = trunk * calc_fourier_seriesy(heading)
   (u, v) = posn
   newpos = (u + delta_x, v + delta_y)
   gradd = ((127+(127*math.sin(((heading*1j)).real))))
   gradd2 = ((127+(127*math.cos(((heading*1j)).real))))
   gradd3 = ((127+(127*math.sin(((heading*1j)).imag))))
...
def draw_tree12xy(arr,rendord,inord, order, theta, thetab, sz, clol, clol2, posn, heading, color=(0,0,0), depth=0):
   trunk_ratio = 1.6180327868852      # How big is the trunk relative to whole tree?
   trunk = sz * trunk_ratio # length of trunk
   delta_x = trunk * calc_fourier_seriesx(heading) #+ math.cos(3*(((heading*1j)).imag)))
   delta_y = trunk * calc_fourier_seriesy(heading)
   (u, v) = posn
   newpos = (u + delta_x, v + delta_y)
   gradd = ((127+(127*math.sin(((heading*1j)).real))))
...
def calc_fourier_seriesy(heading):
    ...
    something = 0
    for x in range(1):
        something += ((math.sin(inc* (((heading*1j)).real)))/inc) 
        inc +=2
    return something
...
def calc_fourier_seriesx(heading):
    ...
    for x in range(1):
        something -= ((math.cos(inc*(((heading*1j)).real)))/inc) 
        inc +=2
    return something
   
def storenewposxx(newpos, color, order):
      ...
      pygame.draw.polygon(main_surface,color, ((newpos[0][0],newpos[0][1]),(newpos[1][0],newpos[1][1]),(newpos[2][0],newpos[2][1])),1)
...
def storenewpos(newpos, color, order):
      ...
      #pygame.draw.circle(main_surface, color, (int(newpos[0][0]),int(newpos[0][1])),1)
      #pygame.draw.circle(main_surface, color, (int(newpos[1][0]),int(newpos[1][1])),1)
      #print()
...
def gameloop():
    ...
    while True:
        ...
        if ev.type == pygame.QUIT:
            ...
            theta1 -= (-math.pi*1j)/100
            theta2 -= (math.pi*1j)/100
            theta12 -= (-math.pi*1j)/100
            theta22 -= (math.pi*1j)/100
