...
def draw_tree(inord, order, theta, thetab, sz, posn, heading, color=(0,0,0), depth=0):
   ...
   if order==1:
   #if True:
      #pygame.draw.line(main_surface, color, posn, posn, order)
      #print(newpos)
      pygame.draw.line(main_surface, color, newpos, newpos, order)
   ...
   gradd  = 128+(127*math.sin((heading).real))
   gradd2 = 128+(64*math.cos(heading.real)+math.cos((heading).imag))
   gradd3 = 128+(64*math.sin(heading.real)+math.sin((heading).imag))
       ...
       if math.fabs(decoded[0]) > 0:
           if math.fabs(decoded[1]) > 0:
               ...
               for i,j in zip(decoded_ohh,decoded_oh):
                  ...
                  theta1 = math.cos(3*math.pi*(i/2147483647))*(1*math.pi*(j/2147483647))#(1*(pygame.mouse.get_pos()[0]/225))
                  #print(pygame.mouse.get_pos()[0]/225)
                  theta2 = 0#math.sin(math.pi*(i/2147483647))*1j#*(1*(pygame.mouse.get_pos()[0]/450))
                  ...
                  draw_tree(inord, 3, theta1, theta2, surface_size*0.15, (500*m,500*m), (math.pi*(i/2147483647)))
                  #draw_tree(inord, 6, theta1, theta2, surface_size*0.15, (500*m,500*m), (math.pi*(i/2147483647)))
                  #draw_tree(inord, 9, theta1, theta2, surface_size*0.15, (500*m,500*m), (math.pi*(i/2147483647)))
                  #draw_tree(inord, 10, theta1, theta2, surface_size*0.3, (500*m,500*m), (math.pi*(i/2147483647)))
