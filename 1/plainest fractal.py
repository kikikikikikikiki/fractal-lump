...
def draw_tree(order, order2, theta, thetab, sz, posn, heading, color=(0,0,0), depth=0):

   
   trunk_ratio = 0.1618       
   trunk = sz * trunk_ratio
   delta_x = trunk * math.sin(heading)
   ...
   gradient = ((255/order2)*order)#-(order**2)
   gradient2 = order**2
   ...
   if order > 0:
      ...
      color1 = (0,0,0)
      color2 = (0,0,0)
      draw_tree(order-1, order2, theta, thetab, newsz, newpos, (heading+theta), color1, depth+1)
      draw_tree(order-1, order2, theta, thetab, newsz, newpos, (heading-theta), color2, depth)
...
def gameloop():
    ...
    while True:
        ...
        draw_tree(order, order, theta1, theta2, surface_size*1, (800,1000), math.pi)
