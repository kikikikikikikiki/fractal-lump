...
def draw_tree(deapth, rendord,inord, order, theta, thetab, sz, clol, clol2, posn, heading, color=(0,0,0), depth=0):
   ...
   if order == 1:
      ...
      #pygame.draw.circle(main_surface, color, (int(circcentre2[0]),int(circcentre2[1])), int(length),int(length))
   if order<rendord:
      pygame.draw.line(main_surface, (0,0,0), posn, newpos, order)
      #pygame.draw.line(main_surface, (0,0,0), posn, newpos2, order)
